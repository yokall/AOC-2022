...
sub _visible_from_top {
    ...
    for ( my $y = $start_y - 1 ; $y >= 0 ; $y-- ) {
        return 0
          if (
            _tree_is_same_height_or_taller( $height_map, $x, $y, $tree_height )
          );
    }
...
sub _tree_is_same_height_or_taller {
    my ( $height_map, $x, $y, $tree_height ) = @_;

    return $height_map->[$y]->[$x] >= $tree_height;
