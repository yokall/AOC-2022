...
my @monkeys = (@starting_monkeys);

for ( my $r = 0; $r < 20; $r++ ) {
    my $monkeys_ref = run_round( \@monkeys, $mod, 0 );
...
for ( my $r = 0; $r < 10000; $r++ ) {
    my $monkeys_ref = run_round( \@monkeys, $mod, 1 );
...
$monkey_business_level = $inspection_counts[0] * $inspection_counts[1];

print 'Part 2 answer: ' . $monkey_business_level . "\n";

sub run_round {
    my $monkeys_ref = shift;
    my $mod         = shift;
    my $part2       = shift;

    my @monkeys = @{$monkeys_ref};
