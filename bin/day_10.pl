...
my $puzzle_input = Common::FileReader::read_file_to_array(
    dirname(__FILE__) . '/../files/10_puzzle_input.txt' );
...
my $cycles = CRT::Cycles::model($puzzle_input);

my $sum = CRT::Cycles::get_value_at_cycle( $cycles, 20 );
$sum += CRT::Cycles::get_value_at_cycle( $cycles, 60 );
$sum += CRT::Cycles::get_value_at_cycle( $cycles, 100 );
$sum += CRT::Cycles::get_value_at_cycle( $cycles, 140 );
$sum += CRT::Cycles::get_value_at_cycle( $cycles, 180 );
$sum += CRT::Cycles::get_value_at_cycle( $cycles, 220 );

print 'Part 1 answer: ' . $sum . "\n";

my $register_cycle_values = CRT::Cycles::get_values_for_all_cycles($cycles);

my $screen_buffer = CRT::Screen::render($register_cycle_values);

foreach my $row ( @{$screen_buffer} ) {
